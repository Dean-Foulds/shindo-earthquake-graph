  ...
  const runAnalysis = async (lat, lon, currentMag, currentDep) => {
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({model:"claude-sonnet-5",max_tokens:4096,
...
ALWAYS give a top-level estimated_casualties (expected fatalities from ALL hazards combined — shaking, building collapse, landslide, fire, tsunami), never null. For shaking-dominant inland events this is driven by collapse and landslide, not tsunami. Also give historical_analogs[].deaths for every analog.`}]})
      ...
      console.log("[Shindo raw response]", t); console.log("[Shindo stop_reason]", d.stop_reason); console.log("[Shindo raw response]", t); console.log("[Shindo stop_reason]", d.stop_reason); setAna(JSON.parse(t))
    }catch(err){
